...
class TreeTable {
  ...
  renderNode(node, depth) {
    ...
    row.title = this.columns
      .map((col) => `${col.header}: ${node[col.key]}`)
      .join("\n");
  ...
  updateSortIndicators() {
    ...
    headers.forEach((header) => {
      ...
      if (this.sortKey === key) {
        ...
        // Ensure class is removed if column is not the sort key
        iconContainer.classList.remove("sort-icon--asc");
      }
    });
  }
    ...
    return data
      .map((node) => {
        const children = node.children
          ? this.filterData(node.children, query)
          : null;
        if (
  ...
  _sortData() {
    ...
    const direction =
      this.options.sortDirection[this.sortKey] === "asc" ? 1 : -1;
    const column = this.columns.find((c) => c.key === this.sortKey);
    if (!column) return;
  ...
  sort(key) {
    if (this.sortKey === key) {
      this.options.sortDirection[key] =
        this.options.sortDirection[key] === "asc" ? "desc" : "asc";
    } else {
      this.sortKey = key;
